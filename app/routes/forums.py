...
def forums():
    ...
    if is_admin:
        ...
        for role in current_user.roles:
            role_thread_ids = [thread_id[0] for thread_id in ThreadRoles.query.with_entities(
                ThreadRoles.thread_id).filter_by(role_id=role.id).all()]
...
def post(post_id, thread_id):
    ...
    return render_template('forums/post.html', comment_authors=comment_authors, post_id=post_id,
