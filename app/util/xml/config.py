...
with open('C:\\Users\\trano\\Documents\\tobu\\pengin-pi\\app\\static\\robots.txt','r') as file:
    def parse(line):
        out = re.search(':', line)
...
#     about.text = f"{protocol}://{domain}/about"
...
#     forum.text = f"{protocol}://{domain}/forums"
...
def load_defaults(url=domain, protocol='https'):
    routes = ['', 'about', 'forums', 'products', 'blogs']
    priorities = ['1.0', '0.5', '0.1', '1.0', '0.9']
    frequencies = ['weekly', 'weekly', 'daily', 'daily', 'daily']
    for i in range (len(routes)):
        url = ET.Element('url')
        loc = ET.Element('loc')
        loc.text = f"{protocol}://{domain}/{routes[i]}"
        lastmod = ET.Element('lastmod')
        lastmod.text = datetime.utcnow().strftime("%Y-%m-%d")  # update the lastmod to current date
        changefreq = ET.Element('changefreq')
        changefreq.text = f"{frequencies[i]}"
        priority = ET.Element('priority')
        priority.text = f"{priorities[i]}"
        url.append(loc)
        url.append(lastmod)
        url.append(changefreq)
        url.append(priority)
...
def load_blogs(blogs,url=domain, protocol='https'):
    for blog in blogs:
        url = ET.Element('url')
        loc = ET.Element('loc')
        loc.text = f"{protocol}://{domain}/products/{blog.id}"
        lastmod = ET.Element('lastmod')
        lastmod.text = datetime.utcnow().strftime("%Y-%m-%d")  # update the lastmod to current date
        # lastmod.text = datetime.utcnow().strftime("%Y-%m-%d")  # update the lastmod to current date
        changefreq = ET.Element('changefreq')
...
def load_products(products, url=domain, protocol='https'):
    for product in products:
        url = ET.Element('url')
        loc = ET.Element('loc')
        # loc.text = f"{protocol}://{domain}/products/{product.name}"
        loc.text = f"{protocol}://{domain}/products/{product.id}"
        lastmod = ET.Element('lastmod')
        lastmod.text = datetime.utcnow().strftime("%Y-%m-%d")  # update the lastmod to current date
        changefreq = ET.Element('changefreq')
    ...
    if new:
        for URL in n_root.iter('url'):
            if name in URL.find('loc').text:
                URL.find('loc').text = f"{protocol}://{domain}/{PATH}/{new}"
                URL.find('loc').text = datetime.utcnow().strftime("%Y-%m-%d")
    else:
        for URL in n_root.iter('url'):
            if name in URL.find('loc').text:
                URL.find('loc').text = f"{protocol}://{domain}/{PATH}/{name}"
                URL.find('loc').text = datetime.utcnow().strftime("%Y-%m-%d")
    n_tree.write(path)
