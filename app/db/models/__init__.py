...
Application.user_id = db.Column(
    db.String(36), db.ForeignKey("user.id", ondelete="CASCADE")
)
Application.job_id = db.Column(
    db.String(36), db.ForeignKey("job.id", ondelete="CASCADE")
)
Application.status_code_id = db.Column(db.String(36), db.ForeignKey("status_code.id"))

# User Roles
UserRoles.user_id = db.Column(
    db.String(36), db.ForeignKey("user.id", ondelete="CASCADE")
)
UserRoles.role_id = db.Column(
    db.String(36), db.ForeignKey("roles.id", ondelete="CASCADE")
)
# Company Members
CompanyMembers.company_id = db.Column(
    db.String(36), db.ForeignKey("company.id", ondelete="CASCADE")
)
CompanyMembers.user_id = db.Column(
    db.String(36), db.ForeignKey("user.id", ondelete="CASCADE")
)
CompanyMembers.role_id = db.Column(
    db.String(36), db.ForeignKey("roles.id", ondelete="CASCADE")
)
# Role
...
Customer.user_id = db.Column(
    db.String(36), db.ForeignKey("user.id", ondelete="CASCADE"), nullable=False
)
Customer.company_id = db.Column(
    db.String(36), db.ForeignKey("company.id", ondelete="CASCADE"), nullable=False
)
...
# ShippingAddress
ShippingAddress.customer_id = db.Column(
    db.String(36), db.ForeignKey("customer.id", ondelete="CASCADE"), nullable=False
)
# Orders
Orders.orders_list = db.relationship("OrdersList")
Orders.customer_id = db.Column(
    db.String(36), db.ForeignKey("customer.id", ondelete="CASCADE")
)
# Contracts
Contracts.customer_id = db.Column(
    db.String(36), db.ForeignKey("customer.id", ondelete="CASCADE")
)
# Thread
Thread.roles = db.relationship("Role", secondary="thread_roles")
# ThreadRoles
ThreadRoles.thread_id = db.Column(
    db.String(36), db.ForeignKey("thread.id", ondelete="CASCADE")
)
ThreadRoles.role_id = db.Column(
    db.String(36), db.ForeignKey("roles.id", ondelete="CASCADE")
)
# ForumPost
ForumPost.comments = db.relationship("ForumComment")
ForumPost.thread_id = db.Column(
    db.String(36), db.ForeignKey("thread.id", ondelete="CASCADE")
)
ForumPost.author = db.Column(
    db.String(36), db.ForeignKey("user.id", ondelete="CASCADE")
)
# ForumComment
ForumComment.author = db.Column(
    db.String(36), db.ForeignKey("user.id", ondelete="CASCADE")
)
ForumComment.post_id = db.Column(
    db.String(36), db.ForeignKey("forum_post.id", ondelete="CASCADE")
)
# Event
Event.user_id = db.Column(db.String(36), db.ForeignKey("user.id", ondelete="CASCADE"))
Event.organizer = db.Column(db.String(36), db.ForeignKey("user.id", ondelete="CASCADE"))
Event.role = db.Column(db.String(36), db.ForeignKey("roles.id", ondelete="CASCADE"))
Event.role_info = db.relationship("Role", back_populates="event_info", lazy=True)
# TicketForum
TicketForum.user_id = db.Column(
    db.String(36), db.ForeignKey("user.id", ondelete="CASCADE")
)
TicketForum.customer_id = db.Column(
    db.String(36), db.ForeignKey("customer.id", ondelete="CASCADE")
)
# TicketComment
TicketComment.ticket_id = db.Column(
    db.String(36), db.ForeignKey("ticket_forum.id", ondelete="CASCADE")
)
TicketComment.author_id = db.Column(
    db.String(36), db.ForeignKey("user.id", ondelete="CASCADE")
)
# OrdersList
...
# Message
Message.author_id = db.Column(
    db.String(36), db.ForeignKey("user.id", ondelete="CASCADE"), nullable=False
)
Message.author = db.relationship("User", back_populates="messages")
Message.room_id = db.Column(
    db.String(36), db.ForeignKey("room.id", ondelete="CASCADE"), nullable=False
)
